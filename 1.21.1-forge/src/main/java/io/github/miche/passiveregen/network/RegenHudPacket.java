...
public final class RegenHudPacket {
    ...
    public static void send(ServerPlayer player, long outOfCombatTicks, int damageCooldownTicks, boolean regenActive, boolean hungerBlocked, boolean justHealed, float currentHealth, float maxHealth, int maxRegenHealthPercent, boolean nearCampfire, boolean saturationBonus, boolean poisoned, boolean withered) {
        PassiveRegenNetwork.CHANNEL.send(new RegenHudPacket(outOfCombatTicks, damageCooldownTicks, regenActive, hungerBlocked, justHealed, currentHealth, maxHealth, maxRegenHealthPercent, nearCampfire, saturationBonus, poisoned, withered), PacketDistributor.PLAYER.with(player));
    }
    ...
    public static void handle(RegenHudPacket packet, CustomPayloadEvent.Context context) {
        RegenHudState.get().applyPacket(packet.outOfCombatTicks, packet.damageCooldownTicks, packet.regenActive, packet.hungerBlocked, packet.justHealed, packet.currentHealth, packet.maxHealth, packet.maxRegenHealthPercent, packet.nearCampfire, packet.saturationBonus, packet.poisoned, packet.withered);
    }
}
