...
import com.multiflame.passiveregen.client.PassiveRegenClient;
...
@OnlyIn(Dist.CLIENT)
public final class RegenHudRenderer extends AbstractGui {
    ...
    public void onRender(RenderGameOverlayEvent.Post event) {
        ...
        client.getTextureManager().bind(HEART_TEXTURE);
        ...
        if (hungerBlocked) {
            ...
            if (cyclePosMs < 250L) {
                ...
                shake = (float) Math.sin(phase * Math.PI * 6.0F) * 0.6F;
            }
        ...
        if (withered) {
            ...
            float pulse = config.hudRichAnimations
                ...
                : 0.05F + 0.18F * (0.5F + 0.5F * (float) Math.sin(glowT * 2.2F - (float) (Math.PI / 2.0D)));
    ...
    private static void drawGlow(MatrixStack matrices, float alpha, int tint, float opacity, float scale) {
    ...
    private static void drawHeart(MatrixStack matrices, float progress, int tint, float opacity, float fillOpacity) {
        int fillHeight = MathHelper.ceil(16.0F * MathHelper.clamp(progress, 0.0F, 1.0F));
        if (fillHeight > 0) {
    ...
    private static void drawHealFlash(MatrixStack matrices, float alpha, float opacity) {
    ...
    private static void drawSparkle(MatrixStack matrices, RegenHudState state, float opacity) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            int x = Math.round(8.0F + (float) Math.cos(angle) * outerR);
            int y = Math.round(8.0F + (float) Math.sin(angle) * outerR);
            fill(matrices, x, y, x + 1, y + 1, dotColor);
            int innerX = Math.round(8.0F + (float) Math.cos(angle) * innerR);
            int innerY = Math.round(8.0F + (float) Math.sin(angle) * innerR);
            fill(matrices, Math.min(innerX, x), Math.min(innerY, y), Math.max(innerX, x) + 1, Math.max(innerY, y) + 1, lineColor);
        }
    }

    private static void drawTimer(MatrixStack matrices, FontRenderer font, String text, int x, int y, int argb) {
        matrices.pushPose();
