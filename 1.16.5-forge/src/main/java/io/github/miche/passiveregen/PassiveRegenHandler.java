...
public class PassiveRegenHandler implements IPassiveRegenInternals {
    ...
    public void onLivingDamage(LivingDamageEvent event) {
        ...
        if (pvpCooldown >= 0 && event.getSource().getEntity() instanceof PlayerEntity) {
    ...
    private static boolean hasBlockedEffect(net.minecraft.entity.player.PlayerEntity player, java.util.List<? extends String> blockedEffects) {
        if (blockedEffects == null || blockedEffects.isEmpty()) return false;
        for (net.minecraft.potion.EffectInstance inst : player.getActiveEffects()) {
            net.minecraft.util.ResourceLocation id = net.minecraftforge.registries.ForgeRegistries.POTIONS.getKey(inst.getEffect());
            if (id != null && blockedEffects.contains(id.toString())) return true;
        }
        return false;
    }

    private static boolean isDimensionBlacklisted(net.minecraft.entity.player.PlayerEntity player, java.util.List<? extends String> dimensionBlacklist) {
